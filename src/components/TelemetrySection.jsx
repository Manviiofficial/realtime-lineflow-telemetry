...
import HighchartsTelemetry from "./HighchartsTelemetry";
...
function generateTelemetryData(fromDate, toDate) {
  ...
  const diff = Math.floor((end - start) / (1000 * 60 * 5));
...
export default function TelemetrySection() {
  ...
  return (
    <div className="w-full flex flex-col items-center py-2 bg-transparent">
      <div className="w-full max-w-full bg-white/90 rounded-2xl shadow-xl border border-blue-100 px-2 md:px-4 py-4 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col md:flex-row gap-4 items-center justify-center mb-4">
          ...
          <div className="flex flex-col gap-1 w-full md:w-1/3">
            ...
            <input type="date" className="rounded border px-2 py-1 text-blue-500 text-sm" value={fromDate} onChange={(e) => setFromDate(e.target.value)} disabled={showGraph} />
          </div>
          {/* To Date */}
          <div className="flex flex-col gap-1 w-full md:w-1/3">
            <label className="text-xs font-semibold text-gray-700">To Date</label>
            <input type="date" className="rounded border px-2 py-1 text-blue-500 text-sm" value={toDate} onChange={(e) => setToDate(e.target.value)} disabled={showGraph} />
          </div>
          {/* Line Selector */}
          <div className="flex flex-col gap-1 w-full md:w-1/3">
            ...
            <Select
              ...
              styles={{
                ...
                control: (base) => ({ ...base, color: "#3b82f6", borderColor: "#3b82f6", minHeight: 32, fontSize: 14 }),
                singleValue: (base) => ({ ...base, color: "#3b82f6", fontSize: 14 }),
        ...
        {error && <div className="text-red-600 font-semibold text-xs mb-2">{error}</div>}
        ...
        {confirm && !showGraph && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 flex flex-col items-center gap-2 mb-2">
            ...
            <div className="flex gap-3 mt-1">
              <button className="px-3 py-1 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 text-xs" onClick={handleShowGraph}>
                Yes, show graph
              </button>
              <button className="px-3 py-1 rounded bg-gray-200 text-gray-800 font-semibold hover:bg-gray-300 text-xs" onClick={handleReset}>
                No, re-enter
              </button>
            </div>
          </div>
          ...
          <div className="w-full bg-white/80 rounded-xl shadow p-4 border border-gray-200 mt-2">
            <div className="mb-2 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleReset} className="px-2 py-1 text-xs font-semibold rounded-lg bg-red-100 hover:bg-red-200 text-red-800 border border-red-300 shadow transition flex items-center gap-1 ml-2" aria-label="Reset Graph">
            ...
            {!showTable ? (
              <>
                ...
                <button className="mt-2 px-3 py-1 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 text-xs" onClick={() => setShowTable(true)}>
                  View Data Table
                </button>
              </>
            ) : (
              <div className="mt-2 border border-blue-200 bg-white/95 rounded-lg shadow px-2 py-2 w-full">
                <style>{`
                ...
                <div className="text-xs text-blue-500 mt-1 text-center cursor-pointer underline" onClick={() => setShowTable(false)}>
                  Back to Chart
                </div>
              </div>
            )}
        ...
        {!confirm && !showGraph && (
          <button className="px-4 py-2 rounded bg-blue-600 text-white font-semibold hover:bg-blue-700 mt-2 text-xs" onClick={handleConfirm}>
            Confirm Selection
          </button>
        )}
      </div>
